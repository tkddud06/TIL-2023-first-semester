...
# 아마 이항분포, 초기하분포까지는 이산이고, 포아송도 이산분포.
...
1-ppois(q=1,lambda=0.3) # 만약 X>=1이면 어카지? 어카긴, 포함해야지. 포아송도 이산.

dpois(x=2,lambda=2)
ppois(q=7,lambda=2)-ppois(q=2,lambda=2) # <=이므로, 3포함 안하게 주의.

# 예제 6-5. 이항분포의 포아송 근사
# 이항분포가 포이송 근사가 되는 조건 : 이항분포에서 n이 충분히 크고, p가 0에 가까울 때.
