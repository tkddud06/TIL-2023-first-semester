...
# 너가 유명하고 많은 사람들이 찾는 투어 안내 회사의 투어 가이드 AI라고 가정하고 대답해줘. 너는 많은 사람들이 선호하는 호불호가 갈리지 않는 지역들을 주로 추천할 거야. 답변의 형식은 제한되어있어.  다른 내용은 절대 작성해서는 안 되고, 여행지 이름과, 3줄로 해당 여행지에 대해 상식적인 부분을 알려주는 전반적인 소개를 작성해줘. '((1))'  형식으로 번호를 붙이고, 바로 뒤이어  '::'형식에 이어,  여행지 이름을 적어주고, '//'에 이어 여행지 소개를 적어서 순서대로 작성해 줘. 그리고 마지막으로 해당 기준의 여행 지역 전반에 대한 소개를 한줄로 작성해 줘. 자, 그럼 이제 "도쿄 애니메이션 3박4일 여행" 기준에 맞는 여행지 10가지를 들를 수 있는 경로를 적어줘.
#
# 너가 유명하고 많은 사람들이 찾는 투어 안내 회사의 투어 가이드 AI라고 가정하고 대답해줘. 너는 많은 사람들이 선호하는 호불호가 갈리지 않는 지역들을 주로 추천할 거야. 답변의 형식은 제한되어있어.  다른 내용은 절대 작성해서는 안 되고, 여행지 이름과, 무조건 3줄로 해당 여행지에 대해 상식적인 부분을 알려주는 전반적인 소개를 작성해줘. '((1))'  형식으로 번호를 붙이고, 바로 뒤이어  '::'형식에 이어,  여행지 이름을 적어주고, '//'에 이어 여행지 소개를 적어서 순서대로 작성해 줘. 단, 해당 형식들은 1번씩만 사용할 수 있어. 그리고 마지막으로 해당 기준의 여행 지역 전반에 대한 소개를 한줄로 작성해 줘. 자, 그럼 이제 "도쿄 쇼핑 3박4일 여행" 기준에 맞는 여행지 10가지를 들를 수 있는 경로를 적어줘.
...
# Write a number in the format of '((1))', immediately followed by '::' format,

# 필요시 메시지 수정 필요. 응답시 띄어쓰기가 생겼다 안생겼다 함.
# 종종 갑자기 앞쪽에 번호를 붙여주거나, 개행을 두번하다가 한번하는 등의 문제가 생김.
# 개행을 두번하다가 한번하는 문제, 마지막에 뜬금없이 //를 붙이는 문제
...
#
# Assume that you are a tour guide AI from a famous and sought-after tour guide company, and answer. You will mainly recommend areas that many people do not dislike. The format of responses is limited. You should never write anything else, and immediately write the name of the destination and an overall introduction that tells you common sense about the destination for three lines. Things you shouldn't write include writing numbers in sequence before the name of the destination. just immediately write the name of the destination, followed by '//' and immediately write the introduction of the destination in order. you should not write any spacing between the name of the destiation, '//', and the introduction of the destination. However, each type can only be used once. After filling out each destination, open it once and continue to fill it out. And lastly, please write a one-line introduction to the entire travel area for that standard.
# You must write for each destination, in the appropriate format as presented: 'Name of destination//about destination' followed by a single space, and then repeat. You must never deviate from that format, or include anything else.
...
# 아래는 데이터 처리 부분
#
# textdata = """((1))::아사쿠사//아사쿠사는 센소지 절과 같은 유서 깊은 사찰과 신사, 전통 상점가인 나카미세도리로 유명한 도쿄의 전통 지역으로 전통 물품과 기념품을 구입할 수 있습니다.
#
# ((2))::시부야//시부야는 트렌디한 패션 쇼핑으로 유명한 도쿄의 번화한 지역입니다. 일본 및 해외 유명 브랜드의 백화점, 부티크, 플래그십 스토어가 많이 있습니다.
#
# ((3))::하라주쿠//하라주쿠는 독특한 패션 문화로 유명한 도쿄의 동네입니다. 다케시타 거리는 화려하고 특이한 의류, 액세서리, 스낵 등을 판매하는 가게가 즐비한 하라주쿠의 인기 쇼핑 거리입니다.
#
# 전체적으로 소개합니다: 도쿄는 쇼핑객에게 다양한 옵션을 제공하는 쇼핑 천국입니다. 전통적인 기념품부터 트렌디한 패션까지, 모든 여행객을 위한 모든 것이 있습니다. 이 3박 4일 여행은 도쿄 최고의 쇼핑 명소로 여러분을 안내합니다."""
#
#
# textdata = """Harajuku// Known for its street fashion and trendy shops, Harajuku is a must-visit destination for anyone looking for unique and eye-catching clothing.
#
# Shinjuku// Shinjuku is Tokyo's busiest district and is known for its towering skyscrapers, shopping and entertainment district.
#
# Ginza// Ginza is a high-end shopping district that boasts designer boutiques, luxury department stores, and exquisite dining options.
#
# The four-day Tokyo shopping trip is an exciting and unique experience where you'll be able to immerse yourself in Tokyo's fashion culture while indulging in the best shopping and dining destinations in the city."""
